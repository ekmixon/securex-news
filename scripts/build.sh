...
function build-and-publish-package {
  ...
  S3_ARTIFACT_FOLDER="securex-news"
  ...
  aws s3 cp ./securex-news.tar.gz s3://${ARTIFACTS_BUCKET}/artifacts/${S3_ARTIFACT_FOLDER}/${ARTIFACT_NAME} --sse aws:kms --sse-kms-key-id alias/kms-s3

  # Run Vulnerability Scan in the artifact using ZeroNorth - INT only
  if [ "${PKG_TYPE}" == "int" ]; then
    echo "$DOCKERHUB_PASSWORD" | docker login -u "$DOCKERHUB_USERNAME" --password-stdin
    docker pull zeronorth/owasp-5-job-runner
    docker run -v /home/travis/build/threatgrid/securex-news:/code/ -e CYBRIC_API_KEY="${CYBRIC_API_KEY}" -e POLICY_ID=JoJaSviQSdSrX2LdiM2KDA -e WORKSPACE=/home/travis/build/threatgrid/securex-news -v /var/run/docker.sock:/var/run/docker.sock --name zeronorth zeronorth/integration:latest python cybric.py
