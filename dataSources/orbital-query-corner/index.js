...
/**
 * Fetch RSS from Cisco Community Blog "Orbital Query Corner" and convert to JSON.
 * @return {Object}
 */
exports.getNews = async () => {
  try {
    const response = await fetch(
      "https://community.cisco.com/api/2.0/search?q=select view_href, subject, body, post_time from messages where subject MATCHES '\"Orbital Query Corner\"' order by post_time desc"
    );

    const rawNews = await response.json();

    return {
      items: rawNews.data.items.map((x) => ({
        title: x.subject,
        link: x.view_href,
        date: x.post_time,
        summary: x.body.replace(/<[^>]*>?/gm, " ").substr(0, 500) + "...",
        sourceId: "ORBITAL",
      })),
      sources: {
        ORBITAL: {
